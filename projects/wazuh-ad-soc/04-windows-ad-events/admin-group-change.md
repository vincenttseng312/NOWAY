...
- `110130`：Sysmon ProcessCreate 偵測群組成員新增命令，僅代表 attempt。
- `110131`：Security 4732 且 `targetSid=S-1-5-32-544`，代表成功加入本機 Administrators。
- `110132`：Security 4728 且群組 SID RID 為 `512`，代表成功加入 Domain Admins。
- Artifact：`code/2026-07-20/wazuh-windows-threat-detection-rules/windows-threat-detection-rules.xml`。

群組名稱可能本地化，因此成功規則優先使用穩定 SID；實際 Wazuh 欄位仍須由 alert JSON 核對。
